...
#if defined(NDEBUG)
# error "Zcashdarklite cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x674302861cea42c9a2d4de81084662215606e2d91829b868089c16182005fa68");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zcashdarklite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zcashdarklite Signed Message:\n";
...
{
    // Zcashdarklite: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zcashdarklite: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Zcashdarklite: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Zcashdarklite: 30 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zcashdarklite: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zcashdarklite: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x674302861cea42c9a2d4de81084662215606e2d91829b868089c16182005fa68");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "1516167418 - ZCDL FOI CRIADO";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043384710fa689ad5023690c80f3a49c8f13f8d45b12382fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce60112435470216fe1b51850b4acf127499c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516167418;
        ...
        assert(block.hashMerkleRoot == uint256("0x4264a2917b5173e2907cfd6f662af23df079dde7354bcc72081002635c881d66"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Zcashdarklite: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZcashdarkliteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZcashdarkliteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZcashdarkliteMiner(CWallet *pwallet)
{
    printf("ZcashdarkliteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zcashdarklite-miner");
    ...
    try { loop {
        ...
        printf("Running ZcashdarkliteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZcashdarkliteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZcashdarkliteMiner, pwallet));
}
